...
class FinishedItem:
    ...
    def __repr__(self) -> str:
        return f'{self.rule[0]} -> {" ".join(self.rule[1:])}\t({self.end})'
...
class EarleyParser:
    ...
    def build_grammar(self):
        ...
        for i, rule in enumerate(self.grammar):
            ...
            self.rule_to_index[rule] = i

    def build_chart(self, input_string):
        self.chart = [OrderedSet([]) for _ in range(len(input_string) + 1)]
        for rule in self.grammar_dict[self.start_symbol]:
            self.chart[0].add(EarleyItem(rule, 1, 0, self.rule_to_index[rule]))
        ...
        for i in range(N + 1):
            ...
            while j < len(self.chart[i]):
                ...
                if not terminal(focus):
                    ...
                    for rule in rules:
                        self.chart[i].add(EarleyItem(rule, 1, i, self.rule_to_index[rule]))
                else:
                    # scan
                    if i < N:
                        ch = input_string[i]
